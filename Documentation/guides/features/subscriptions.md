...
- This example should be reviewed, and modified to make sure it work for your use case so there is no underlying issues
...
struct RedisPubSub: PubSub {
    ...
    actor Dispatcher {
        private var broadcasting: [String: Broadcast<String>] = [:]

        private func subscribe(to channel: String) async -> Broadcast<String> {
        ...
        private func applySubsription(from channel: RedisChannelName, to broadcast: Broadcast<String>) async {
            do {
                // Try to subscribe and pass to broadcast
                try? await redis.subscribe(
                    to: channel,
                    messageReceiver: { _, msg in
                        guard let event = msg.string else { return }
                        Task {
                            await broadcast.publish(event)
                        }
                    },
        ...
        private func publish(for channel: String, _ value: String) async {
    ...
    // Only for string, will not work for non string object, have generic to conform to only
    public func asyncStream<DataType: Sendable>(_ type: DataType.Type = DataType.self, for trigger: String) -> AsyncStream<DataType> {
        guard type == String.self else { return AsyncStream<DataType> { $0.finish() }}
    ...
    public func publish(for trigger: String, payload: Sendable) async {
        guard let payload = payload as? Encodable else { return }
...
struct Resolver {
    ...
    func onCreate(ctx: Context, _: NoArguments) async -> EventStream<Message> {
        let source = ctx.pubsub.asyncStream(String.self, for: "message-create")
        return AsyncStream<Message> { con
            let task = Task {
                for await event in source {
                    guard let message = try? JSONDecoder().decode(Message.self, from: event) else { continue }
                    con.yield(message)
                }
                con.finish()
            }
