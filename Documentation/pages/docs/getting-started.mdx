...
Now, let's add the subscription resolver. Pioneer can resolve subscription as long as the return value is an `AsyncSequence`
...
struct Resolver {
    ...
    func bookAdded(ctx: Context, args: NoArguments) -> AsyncStream<Book> {
        pubsub.asyncStream(Book.self, for: trigger)
    }
}
```
