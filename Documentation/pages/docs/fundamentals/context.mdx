...
```swift {2-11} showLineNumbers copy
...
```swift filename="main.swift" {8-10} showLineNumbers copy
import Vapor
import Pioneer
...
<details>
<summary> Passing down through a context builder for [Vapor](https://github.com/vapor/vapor) </summary>
...
func onOrder(ctx: Context, args: NoArguments) -> AsyncStream<Order> {
    ctx.pubsub.asyncStream(Order.self, for: "*:order")
}
```