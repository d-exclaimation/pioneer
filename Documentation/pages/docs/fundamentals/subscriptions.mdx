...
Resolvers for Subscription differ from resolvers for fields of other types. Specifically, Subscription resolvers require to return an implementation of `AsyncSequence`.

```swift {3-11,14-16} filename="Resolver.swift" showLineNumbers copy
struct Resolver {
    // Example using an async stream generator
    func hello(_: Context, _: NoArguments) async -> AsyncStream<String> {
    ...
    func hello(_: Context, _: NoArguments) async -> AsyncStream<String> {
        pubsub.asyncStream(String.self, for: "HELLO_MESSAGE")
    }
}
```

For the subscription to work under [Pioneer](/) the resolver function must return a type that conforms to `AsyncSequence`.
